...
class AuthController extends AbstractController
{
    ...
    public function test(ServerRequestInterface $request, ResponseInterface $response)
    {
    	...
    	for ($i=1; $i < count($tab); $i++) { // On fait un boucle afin que chaque index du tableau soit vérifié
    		if (abs($tab[$i]) < abs($tab[$idMinAbs])) { // On fait une condition afin de comparé la valeur absolue du chiffre a un index par rapport a la valeur absolue de $idMinAbs
    			$idMinAbs = $i; // Dans le cas ou la condition est vérifiéon stock le résultat
    		}
    	}

        return $this->twig->render($response, '/testValeurAbs.twig', ['idMinAbs' => $tab[$idMinAbs]]); // On envoie le tout a notre page "view", attention a bien envoyer $tab[$idMinAbs] et non pas juste $idMinAbs
    }
}